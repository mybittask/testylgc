...
#if defined(NDEBUG)
# error "logicrypt cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // logicrypt: starting difficulty is 1 / 2^12
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 16500 blocks, which will occur approximately every 2 months
    nSubsidy >>= (nHeight / 16500); // logicrypt: 16.5k blocks in ~2 months

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 16 * 30; // logicrypt: 8 minutes
static const int64 nTargetSpacing = 8 * 30; // logicrypt: 4 minutes
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "FOX News 12DEC2017 The ground shifts on immigration";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("157984710fa689ad5023690c80f3a49a8f13f8d45b8c857fbcbc8bc4a8a4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b15760b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1513710739;
...
void static logicryptMiner(CWallet *pwallet)
{
    printf("logicryptMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("logicrypt-miner");
    ...
    try { loop {
        ...
        printf("Running logicryptMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("logicryptMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&logicryptMiner, pwallet));
}
